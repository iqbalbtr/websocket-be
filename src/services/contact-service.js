...
module.exports = {
    create: async (user, req) => {

        if(user.username === req.username)
            throw new ResponseError(400, "User not permited");
        ...
        const exist = await prisma.contact.count({
            where: {
                contact_list_id: user.contact_id,
                user: {
                    username: req.username
                }
            }
        ...
        const list = [
            ...all
                ...
                .map(foo => ({
                    ...
                    profile: foo.user.user_info.profile,
    ...
    getContactByUser: async(contactListId, username) => {

        const get = await prisma.contact.findFirst({
            where: {
